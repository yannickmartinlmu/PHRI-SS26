...
arm / rail / gripper means strictly sequential use. Every hardware path is
funnelled through one primitive (_move_arm / _gripper / _move_elmo).

_move_arm has two interchangeable backends driven from the SAME joint-angle
table, so switching can never land the arm somewhere different — MoveIt only
adds collision-aware planning on the way to an identical target:
...
# Kitchen collision scene lives in scripts/kitchen_scene.py (single source of
# truth, user-edited); --symlink-install makes realpath resolve to the real
...
# Elmo setpoints (Float32). carriage = base_link -X, lift = Z. Both axes speak
# the identical topic pair, so one primitive drives them both.
ELMO_AXES = ("carriage", "lift")
ELMO_SET = "/elmo/id1/{axis}/position/set"
ELMO_GET = "/elmo/id1/{axis}/position/get"


# Every hand the robot can see — both recognizers feed this one topic, which
# interaction_manager also watches for the open-palm barge-in (see gestures.py).
...
# Sec the arm holds the glass under the tap waiting to be told "done". Bounded so
# a dead transcriber parks the arm at the sink for two minutes, not forever. Long
# because a "wait" from the user now costs a round, not the whole goal.
WATER_CONFIRM_TIMEOUT = 120.0

# Waiting under a tap looks like a hang, so the wait escalates: hold, then wiggle,
# then the lab light. Both nudges are pure feedback and both FAIL OPEN — no light
# node, no planner, the water still happens.
WIGGLE_EVERY = 15.0       # sec of silence before a nudge, and every 15s after —
                          # one nudge in a 2-minute window reads as giving up
BLINK_AFTER = 40.0        # sec before the light joins in
ANSWER_GRACE = 10.0       # sec past the timeout before we give up on the reply.
                          # interaction_manager answers at WATER_CONFIRM_TIMEOUT,
                          # so cancelling at exactly 60 would race its response.
...
WIGGLE_MOVE_TIME = 1.5    # sec per leg; ~4.5s of servo noise inside the listening
                          # window, and the ear stays open throughout
...
# Tunneling knob — arm width catches thinner boxes, shrink if one slips through.


# Base sags under load; the kitchen seen from the tilted base_link is rotated by
# -TILT, which build_scene applies so MoveIt's level-FK check matches reality.
# Measure: jog to a worst-case left reach, tilt ~= tip_drop / reach.
TILT_DEG = 0.0     # 0 until measured; single constant, per-config model later
TILT_AXIS = "y"    # sag plane: reach along X (rail) dips in Z

# Sag model (offline fit, NOT wired yet). Sag is a pitch about Y and torque about
# Y ~ weight * EE x-offset, so tilt is linear in x alone: tilt(x) = a*x + c, and
# the measured drop = tilt*x = a*x^2 + c*x (why drop looked nonlinear).
...
# Cartesian pose goals (move_to_point). Frame E = fixed, origin at carriage=0 /
# lift=0, axes parallel to base_link. Elmo feedback is metres 1:1, so E->base_link
# is pure translation and the carriage nulls X.
RAIL_MIN, RAIL_MAX = -0.6, 1.1      # physical rail travel; widen if it reaches further
GRIPPER_HORIZ_QUAT = (0.5, -0.5, 0.5, -0.5)   # horizontal side grasp; proven as arm_teleop HORIZ
# Verify: jog arm to a level side grasp, `tf2_echo base_link end_effector_link`
...
# Continuous joints (1/4/6 without use_external_cable) get 2*pi — safe either way.
...
POSES = {
    ...
    "look_at_user_question":         [-0.1, 0.0, 1.77, 0.0, -0.2, 1.4],  # slight head-tilt after taking the position
...
# Which fixed camera can see a person. NOT the arm camera: it rides the wrist, so
# what it sees says where the arm is pointed, not where the user is.
PERSON_TOPICS = {
...
def _check_poses():
    ...
    # the arm already holding a glass.
...
def _closest_carriage(px):
    # The carriage that nulls the point's X, clamped to the rail. Negative
    # because the rail axis is inverted vs the arm (elmo-axis-mapping).
    return max(RAIL_MIN, min(RAIL_MAX, -px))


def _point_to_base(px, py, pz, carriage, lift):
    # Frame E -> base_link: pure translation, 1:1 metres. A clamped carriage
    # leaves a nonzero X residual for the arm to stretch to.
...
def fit_tilt_line(rows):
    # rows: (x, y, z, lift, measured_floor_height) from measure() + a tape. Sag is
    # a pitch about Y and the arm sits below the mount, so the Y offset makes no
    # torque and drops out. Fit on drop (well-conditioned), NOT drop/x — that
    # blows up near the forward pose where x ~= 0.
    import numpy as np
    A, drops = [], []
...
# The order query_all_drinks mimes. The suggestion goes first — they already
# heard it named, so it is the shortest path to a thumbs-up.
def menu_from(drink):
...
# A schedule, not a state machine: each (at, action) fires once when the clock
# passes it, and any answer ends the whole thing. Clock and sleep are arguments
# so _check_escalation can run it on a fake clock, instantly.
# Ceiling: `done` is only checked between steps, so a wiggle already under way
# finishes first — deliberate, a half-move looks like a fault.
...
# The sink wait as one flat schedule: a nudge every WIGGLE_EVERY sec, the light
# once, and a last step that only ends the wait.
...
class ArmController(Node):
    ...
    def __init__(self):
        ...
        # The wrist camera is the eye. ask_user blocks on the Event from a skill
        # thread while the executor spins; the string says which way it went.
        ...
        # False until a recognizer says otherwise, which degrades to "assume
        # kitchen" — the direction the arm looked before any of this existed.
        ...
        # base_link rides the rail and MoveIt won't re-transform a cached scene,
        # so re-publish after every Elmo move + seed once at startup. Best-effort.
        ...
        # Live TF for FK readback (measure). Pure rclpy — no numpy.
    ...
    def wait_ready(self, timeout=20.0):
        # DDS discovery over the lab network takes seconds, and an Elmo setpoint
        # published before its subscriber is matched is silently dropped (volatile
        # QoS). Actions gate on wait_for_server(); this is the same gate for the
        # raw Elmo pub/sub.
    ...
    def _on_goal(self, goal_request):
        # Off-menu goals are refused, not routed to a default skill: a wrong drink
        # that LOOKS like it worked costs more than a rejected goal.
        if goal_request.drink not in MENU:
            self.get_logger().warn(
                f"[bring_drink] not on the menu: '{goal_request.drink}' "
                f"(have {', '.join(MENU)})")
            return GoalResponse.REJECT
        # One arm, one goal at a time. Simple flag, not a hard lock — two goals
        # landing in the same instant could both pass.
        if self._busy:
    ...
    def _on_arrival(self, _msg):
        ...
        # the tap, and swinging to the entrance mid-fetch is the one way this can
        # do damage. Drop it, don't queue it.
        if self._busy:
    ...
    def _seed_scene(self):
        # Seed the home scene once, as soon as both Elmo axes have reported.
        if all(v is not None for v in self._elmo_pos.values()):
            self._scene_timer.cancel()
            self._publish_scene()

    def _publish_scene(self):
        # Re-cache the kitchen boxes for wherever the rail is NOW. Best-effort: a
        # scene failure logs and returns, it must never abort a drink.
    ...
    def _send(self, client, goal):
        # Blocking send. Safe only because a MultiThreadedExecutor keeps spinning
        # in another thread — spin_until_future_complete would deadlock here.
    ...
    def _move_arm_fk(self, angles, seconds=FK_MOVE_TIME):
        ...
        # `seconds` is the one timing knob in this node — MoveIt has none, which
        # is why the wiggle calls this directly.
    ...
    def _move_arm_pose(self, x, y, z):
        # The ONE place Cartesian IK lives. MoveIt only — FK has no IK. Position:
        # a small sphere at (x,y,z) in base_link. Orientation: gripper horizontal,
        # azimuth ~free because the bottle is round. Mirrors the proven message
        # shape in scripts/arm_teleop.py make_goal().
        ...
        # HORIZ_QUAT maps EE y -> base vertical (math, not measured), so the free
        # azimuth rides the Y tolerance. Wrong tilt in a test? swap which axis
        # gets POSE_YAW_TOL.
        ...
        # Holds the SAME level orientation for the whole path so a held glass never
        # tips. The arm must ALREADY be level at the start or constrained planning
        # fails instantly, and it is slower — bump the 10s / 16 attempts above if
        # this starts timing out.
    ...
    def _gripper(self, target_pos):
        ...
        # so reached_goal alone would raise on every successful pick.
    ...
    def _on_gesture(self, msg):
        # "none" is most of this topic — it ticks at 10 Hz either way — and it is
        # not an answer. Without this guard every ask_user() returns instantly.
        if msg.data == GESTURE_NONE:
            return
        # The other three all end the wait: sitting out the rest of the timeout
        # after the user has answered reads as a hang.
        self.get_logger().info(f"[gesture] {msg.data}")
        self._gesture = msg.data
        self._answered.set()

    def _wiggle(self):
        # FK directly: this is a wrist twist inside a pose the arm already holds,
        # and FK is the only backend that takes the duration.
        # FAIL-OPEN — a nudge is pure feedback and must never abort a water goal
        # with the glass still under the tap. The bounded wait is part of that:
        # _send's own wait_for_server has no timeout, so a missing controller
        # would park the whole escalation here.
    ...
    def _blink(self, on):
        # Fail-open and fire-and-forget: nothing downstream depends on the light.
    ...
    def _ask_for_water(self):
        ...
        # while this is pending. The ear stays open the entire time, wiggle or not.
        ...
        if not future.done():
            # Only reachable if interaction_manager died holding the request.
            future.cancel()
    ...
    def _move_elmo(self, axis, target_position):
        # float(): skills reach here from the CLI, where every arg is a string.
        target = float(target_position)
        self.get_logger().info(f"[elmo] {axis} -> {target}")
        self._elmo_pub[axis].publish(Float32(data=target))

        # Poll the feedback topic — the Elmo reports position but has no done
        # signal. Blocking is fine: another thread spins.
        deadline = time.monotonic() + ELMO_TIMEOUT
        while time.monotonic() < deadline:
            time.sleep(ELMO_POLL)
            position = self._elmo_pos[axis]
            # None = no feedback yet; arriving is only knowable once one lands.
    ...
    def move_to_point(self, px, py, pz):
        # Reach a point in frame E, gripper horizontal. Carriage slides to null X;
        # lift is READ, not moved (grab by dropping the lift as a separate skill);
        # the arm IKs the rest. CLI args arrive as strings -> float().
    ...
    def _lift_path_clear(self, target):
        # Elmo isn't a MoveIt joint, so MoveIt can't plan the lift. Instead: freeze
        # the arm, virtually move the kitchen boxes to each height it sweeps
        # through, and ask /check_state_validity. is_diff=True + empty state means
        # "the arm where it is right now", and /apply_planning_scene is synchronous
        # so the scene is live before each check.
        ...
        # Every step from current toward target, plus target itself — a partial
        # final step must still be checked (a 0.08 m move checks 0.05 AND 0.08).
    ...
    def test_tilt(self, deg, axis="x", parent="base_link"):
        # POC: broadcast `<parent>_tilted` at 10 Hz so it shows in RViz. NOT the
        # real fix — a lone triad, the arm does not follow it (it is parented to
        # base_link by robot_state_publisher). Proves the plumbing. Ctrl-C to stop.
        import math
    ...
    def measure(self):
        # FK of the EE in base_link RIGHT NOW, no motion. Call it in any pose, then
        # tape the tip's floor-height -> one calibration tuple for the sag fit.
        # This is the *nominal/level* FK (the sag lives above base_link, not in the
        # joints), i.e. the predicted tip to diff against your measured one.
    ...
    def move_and_measure(self, joint_1):
        # Sweep only joint_1 across the fully-extended pose, then measure. FK
        # backend: direct + collision-bypassed on purpose — CLEAR THE WORKSPACE,
        # this is a calibration reach, not a production move.
        a = float(joint_1)
    ...
    def calibrate_tilt(self, *rows):
        ...
        # Prints (a, c) -> paste into TILT_LINE. Take a 4th, held-out pose to
        # confirm: if the drop at a near-forward (x ~= 0) pose isn't ~0, a 2nd axis
        # is leaking in. NOT WIRED yet.
    ...
    def _tilt_now(self):
        # Live pitch (deg) about Y for the CURRENT pose, from the fitted TILT_LINE.
        # Will feed build_scene once TILT_LINE is real. NOT WIRED yet.
        from rclpy.time import Time
        from rclpy.duration import Duration as RclDuration
        tf = self._tf_buffer.lookup_transform(
            "base_link", EE_LINK, Time(), timeout=RclDuration(seconds=2.0))
    ...
    def _ask_human_to_pour(self, drink, why):
        # FAIL-OPEN, like _ask_for_water: no machine must not mean no drink. The
        # arm holds the pose under the spout, says out loud that it is not brewing,
        # and gives a human coffee_skip_wait seconds to do it by hand.
    ...
    def _request_coffee_machine(self, drink):
        ...
        try:
            ...
            # Rejected goal, Home Assistant down, machine busy — all the same to
            # the user: the glass still comes, it just needs a pair of hands.
    ...
    def ask_user(self, timeout=GESTURE_TIMEOUT):
        ...
        # part of the question: the camera rides the wrist, so until it points at
        # the user no thumb can be seen — which is why WHICH way it turns is
        # decided here, off the fixed cameras. Returns the gesture; "" = nobody.
        #
        # The PC pose is the default and the tie-break — it is the swing the arm
        # has always used. Turning to the kitchen needs positive evidence.
        ...
        # seen during the mime would still be latched and the wait would return
        # instantly. Safe to clear late only because this is the state stream — a
        # thumb still held is re-sent within 100 ms.
    ...
    def _execute(self, goal_handle):
        ...
        try:
            if goal_handle.request.offer_menu:
                # The user waved the talking away, so the arm asks instead. Inside
                # the try: the mime is motion and a raised move must abort.
...
def main():
    ...
    if args:
        ...
        # exactly as they do under the action server.
